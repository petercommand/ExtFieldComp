...
expand : ∀ {A o} (n : ℕ)
    ...
    -> NestMod (Expr1 A o) n vec
expand n vec mul target (Let1 expr expr₁)
  = let e = expand n vec mul target expr
        e' = expand n vec mul target expr₁
    in expandLet n vec e e'
expand n vec mul target (LetC1 x expr)
  = let e = expand n vec mul target expr
    in expandLetC n vec x e
expand zero [] mul target (Var1 x) = Var1 x
expand (suc n) (v ∷ vec) (m , mul) (t , target) (Var1 x)
  = replicate (expand n vec mul target (Var1 x))
expand zero [] mul target (Add1 expr expr₁) = Add1 expr expr₁
expand (suc n) (v ∷ vec) mul target (Add1 expr expr₁)
  = let e = expand (suc n) (v ∷ vec) mul target expr
        e' = expand (suc n) (v ∷ vec) mul target expr₁
    in zipWith (expandAdd n vec) e e'
expand zero [] mul target (Mul1 expr expr₁) = Mul1 expr expr₁
expand {_} (suc n) (x ∷ vec) (m , mul) (t , target) (Mul1 expr expr₁)
  = let e1 = expand1 n x vec (m , mul) (t , target) expr
        e2 = expand1 n x vec (m , mul) (t , target) expr₁
    in map (expand n vec mul target) (m _ e1 e2 (t _))
