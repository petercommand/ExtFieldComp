...
fpVerify {n} {p} {{record { compSize = .1 ; toIR = toIR }}} refl expr                          | just (x₁ , x₂) | just (F x)  | nothing = {!!}
fpVerify {n} {p} sp expr                                                                       | just (x , x₁)  | nothing with run' {_} {_} {{p}} {{numFp {n} {p} {{numℕ}}}} [] (proj₁ x₁) (proj₂ x₁)
fpVerify {n} {p} {{record { compSize = .1 ; toIR = toIR }}} refl expr                          | just (x₃ , x₁) | nothing     | just (x ∷ []) = {!!}
fpVerify sp expr                                                                               | just (x , x₁)  | nothing     | nothing = refl
