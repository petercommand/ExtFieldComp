...
%
Again following the categorical style as outlined by Bird and de
Moor~\cite{DBLP:books/daglib/0096998}, we consider the following
...
%
To see that $expr\ R$ does represent polynomials from the ring of
polynomials $R[X]$ whose coefficients are from a ring $R$, we can
think of $expr\ R$ as the datatype for a special kind of binary trees.
% 
Such kind of binary tree can have two different types of leaf nodes,
...
gives catamorphisms via $fold$,
i.e., \begin{equation} \label{eq:catamorphism}
  \cata{f_i,f_\ell,f_a,f_m}=fold\ (f_i,f_\ell,f_a,f_m) : expr\
  R\rightarrow S, \end{equation} where $f_i : 1\rightarrow S$,
